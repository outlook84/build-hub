...
ydv_opts = {
    ...
    "addmetadata": True,
    "noplaylist": True,
    "outtmpl": "downloads/ytdl/videos/%(title)s.%(ext)s",
}
...
def download_video(url):
    ...
    try:
        ...
        if is_youtube_url:
            ...
            file_path = f"downloads/ytdl/videos/{title}.mp4"
...
async def ytvm(client: Client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("Please provide a URL to download.")
...
modules_help["ytdl"] = {
    "ytv [name|link]": "Download Video from various sites.",
